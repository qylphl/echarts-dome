<template>
  <div class="chart-content">
    ...
    <div class="nav-main">
      ...
      <p class="nav-span" v-for="(msg, index) in listData" :key="index">
        <span :class="{ 'nav-link': true, 'nav-active': true }" @click="canvasnav()._toggle(index)">
          <i class="datav-font"></i>{{ msg }}
        </span>
      </p>
    </div>
  </div>
...
export default {
  ...
  methods: {
    canvasnav() {
      ...
      var opt = {};
      opt.currentIndex = 1;
      opt.canvas = this.$refs.canvasContent;
